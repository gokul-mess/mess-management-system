...
// This component is used in students-list.tsx but defined here for co-location
export function MealPlanBadge({ plan }: { plan?: 'L' | 'D' | 'DL' }) {
  if (!plan) return <span className="text-xs text-muted-foreground">Not set</span>
  const config = {
    L:  { label: 'Lunch Only',  className: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300' },
    D:  { label: 'Dinner Only', className: 'bg-purple-100 text-purple-700 dark:bg-purple-900/30 dark:text-purple-300' },
